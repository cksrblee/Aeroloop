...
class VSPAeroRunner:
    ...
    def run_vspaero_sweep(self, 
        ...
        # Set StopBeforeRun to generate files without launching vspaero inside C++
        self.vsp.SetIntAnalysisInput(sweep_name, "StopBeforeRun", [1], 0)
        self.vsp.Update()
        
        # This will write AC.vspaero, AC.vspgeom, etc. but will immediately return
        self.vsp.ExecAnalysis(sweep_name)
        
        # Now we launch vspaero manually using Python's subprocess to avoid waitpid bugs!
        import subprocess
        vspaero_exe = self.vspaero_path
        if os.path.isdir(vspaero_exe):
            vspaero_exe = os.path.join(vspaero_exe, "vspaero")
            
        # Get base name of the model from the current working directory state
        # The sweep creates files matching the current VSP model name
        if base_name is None:
            import glob
            vspaero_files = glob.glob("*.vspaero")
            if not vspaero_files:
                raise RuntimeError("VSPAERO setup files were not generated by OpenVSP!")
                
            base_name = os.path.splitext(vspaero_files[0])[0]
        
        import multiprocessing
        num_threads = str(min(8, multiprocessing.cpu_count()))
        
        # Launch vspaero manually. It writes to stdout which we can pipe to the redirect file
        out_stream = subprocess.DEVNULL
        if redirect_file:
            out_stream = open(redirect_file, "w")
            
        subprocess.run([vspaero_exe, "-omp", num_threads, base_name], cwd=".", stdout=out_stream, stderr=subprocess.STDOUT, check=True)
        
        if redirect_file:
            out_stream.close()

        # We return a dummy sweep_res_id since we didn't run it through Results Manager
        return "MANUAL_VSPAERO_RUN"
    ...
    def run_vsploads(self, base_name: str, cwd: str = ".") -> bool:
        ...
        vsploads_exe = os.path.join(self.vspaero_path, "vsploads")
